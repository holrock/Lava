...
sbdReplyType 
start_ajob (struct jData *jDataPtr, struct qData *qp, struct jobReply *jobReply)
{
...
signal_job (struct jData *jp, struct jobSig *sendReq, struct jobReply *jobReply)
{
    ...
    reply = callSBD (toHost, request_buf, XDR_GETPOS(&xdrs), &reply_buf, &hdr, 
		     NULL, NULL, hostData, lastHost, fname, &errcnt, &cc,
		     CALL_SERVER_NO_WAIT_REPLY | CALL_SERVER_NO_HANDSHAKE,
		     &sbdNode, &socket);
    xdr_destroy(&xdrs);
...
msg_job (struct jData *jp, struct Buffer *mbuf, struct jobReply *jobReply)
{
    ...
    reply = callSBD (toHost, mbuf->data, mbuf->len, &reply_buf, &hdr, 
		     NULL, NULL, hostData, lastHost, fname, &errcnt, &cc,
		     0, NULL, NULL);
...
probe_slave (struct hData *hData, char sendJobs)
{
    ...
    errcnt = -10;       
    reply = callSBD(toHost, request_buf, XDR_GETPOS(&xdrs), &reply_buf, &hdr, 
		    NULL, NULL, hData, lastHost, fname, &errcnt, &cc,
		    CALL_SERVER_NO_WAIT_REPLY | CALL_SERVER_NO_HANDSHAKE,
		    &sbdNode, sockPtr);
    xdr_destroy(&xdrs);
...
rebootSbd (char *host)
{
    ...
    reply = callSBD(host, request_buf, XDR_GETPOS(&xdrs), &reply_buf, &hdr, 
		    NULL, NULL, hData, lastHost, fname, &errcnt, &cc, 0,
		    NULL, NULL);
    xdr_destroy(&xdrs);
	...
	int callServerFlags, struct sbdNode *sbdPtr, int *sockPtr)
{
    ...
    *cc =call_server (toHost, sbd_port, request_buf, len, reply_buf, 
		      replyHdr, connTimeout, 60*30,
		      sockPtr, postSndFunc, postSndFuncArg,
		      callServerFlags);
    if (*cc < 0) {
        if (*cc == -2) { 
...
callSbdDebug(struct debugReq *pdebug)
{
    ...
    static char fname[]="mbd.serv: callSbddebug()";
    sbdReplyType reply;
    char request_buf[MSGSIZE];
    char *reply_buf;
    XDR xdrs;
   ...
   if ((hData = getHostData(pdebug->hostName)) == NULL) {
    ...
    reply = callSBD(pdebug->hostName, request_buf, XDR_GETPOS(&xdrs),&reply_buf,
		    &hdr, NULL, NULL, hData, lastHost, fname, &errcnt, &cc, 0, 
		    NULL, NULL);
    xdr_destroy(&xdrs);
...
#ifdef INTER_DAEMON_AUTH
static int
getMbdAuth(struct lsfAuth *auth, char *toHost)
{
    char buf[1024];

    sprintf(buf, "mbatchd@%s", clusterName);
    return getDaemonAuth(auth, toHost, buf, "sbatchd");
} 

int
getDaemonAuth(struct lsfAuth *auth, char *toHost, char *client,
              char *server)
{
    static char fname[] = "getDaemonAuth";
    int rc;
...
void 
writeEauthAuxData(struct lenData *aux_auth_data)
{
    static char fname[] = "writeEauthAuxData";
    ...
    if (aux_auth_data->len) {
        ...
        aux_file = createTmpFileName(".axmd");
        aux_fd = open(aux_file, O_WRONLY|O_EXCL|O_CREAT,
                      S_IRUSR|S_IWUSR|S_IRGRP|S_IROTH);
        if (aux_fd >= 0) {
            if (write(aux_fd, aux_auth_data->data, aux_auth_data->len)
                == aux_auth_data->len) {
            ...
            ls_syslog(LOG_ERR, I18N_FUNC_FAIL_M, fname, "open");

        free(aux_auth_data->data);
    }
}

void
removeFile(const char *envVar)
{
    char *aux_file = getenv(envVar);
    if (aux_file != NULL && strlen(aux_file) != 0) {
        unlink(aux_file);
    }
}
#endif 
